...
router.post("/telecaller/addorder/:id", async (req, res) => {
  try {
    ...
    newOrder.orderedBy.role = "telecaller";
    newOrder.orderedBy.name = telecaller?.name ?? "N/A";
    newOrder.orderedBy.email = telecaller?.email ?? "N/A";
    newOrder.orderedBy.contact = telecaller?.mobile ?? "N/A";
...
router.get("/get/pincodes", async (req, res) => {
  try {
    ...
    const pincodes = await Pincodes.find();
    const PINCODES =
      pincodes && pincodes.length > 0 && pincodes.map((item) => item.pincode);
